...
import {
  quarters, quarterReports, customers, customerQuarterProfiles, feedbackAssignments,
  getFeedbackResponses, questions, getScoreColor, sectionLabels,
  getOutcomeLabel, getOutcomeBadgeClasses,
} from '@/data/mockData';
...
export default function ReportDetailPage() {
  ...
  const report = quarterReports.find(r => r.quarterId === quarterId);

  const submittedAssignments = useMemo(() =>
    feedbackAssignments.filter(a => a.quarterId === quarterId && a.status === 'submitted'),
    [quarterId]
  );
  ...
  const respondents = useMemo(() =>
    submittedAssignments.map(a => {
      ...
      const profile = customerQuarterProfiles.find(p => p.customerId === a.customerId && p.quarterId === quarterId);
      const responses = getFeedbackResponses(a.id);
      return { assignment: a, customer, profile, responses };
    }),
    [submittedAssignments, quarterId]
  );
  ...
  const sectionData = sectionKeys.map(key => {
    ...
    const applicableRespondents = isNewOnly
      ? respondents.filter(r => r.profile?.expatType === 'new')
      : respondents;
    const respCount = applicableRespondents.length;
    let totalScore = 0;
    applicableRespondents.forEach(r => {
      sectionQuestions.forEach(q => {
        const resp = r.responses.find(res => res.questionId === q.id);
  ...
  const strongest = [...sectionData].sort((a, b) => b.pct - a.pct)[0];
  const weakest = [...sectionData].sort((a, b) => a.pct - b.pct)[0];
    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm -mx-6 px-6 py-3 border-b border-border -mt-2">
        <div className="flex items-center gap-3">
          ...
          <span className={cn('text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wide ml-auto', getOutcomeBadgeClasses(report.outcome))}>
                ...
                return (
                  <>{/* Section header row */}
                    <TableRow key={`header-${sectionKey}`}>
                      <TableCell colSpan={respondents.length + 1} className="bg-muted/40 font-semibold text-xs uppercase tracking-wide text-muted-foreground py-2">
                    ...
                    {sectionQuestions.map(q => (
                      <TableRow key={q.id}>
                        <TableCell className="sticky left-0 bg-card z-10 text-xs">
                          <span className="font-mono text-muted-foreground mr-1.5">Q{q.number}</span>
                          {q.text.length > 50 ? q.text.substring(0, 50) + '…' : q.text}
                        </TableCell>
                        {respondents.map(r => {
                          const isNew = r.profile?.expatType === 'new';
                          if (q.isNewExpatOnly && !isNew) {
                            return <TableCell key={r.assignment.id} className="text-center"><span className="text-muted-foreground/20">—</span></TableCell>;
                          }
                          const resp = r.responses.find(res => res.questionId === q.id);
        ...
        <Collapsible open={questionRefOpen} onOpenChange={setQuestionRefOpen}>
          ...
          <CollapsibleContent>
            <div className="mt-3 space-y-4">
              {sectionKeys.map(key => (
                <div key={key}>
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">{sectionLabels[key]}</h4>
                  <div className="space-y-1.5">
                    {questions.filter(q => q.section === key).map(q => (
                      <div key={q.id} className="flex gap-2 text-sm">
                        <span className="font-mono text-muted-foreground w-6 shrink-0">Q{q.number}</span>
            ...
            <TableBody>
              {sectionData.map(s => (
                <TableRow key={s.key}>
                  ...
                  <TableCell className="text-muted-foreground text-sm font-mono">{s.respCount} × {s.qCount} = {s.divisor}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
              ...
              {sectionData.map(s => {
                ...
                return (
                  <TableRow key={s.key}>
                    ...
                    <TableCell>
                      <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-full', getOutcomeBadgeClasses(outcomeBadge))}>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <div className="bg-card rounded-xl border border-border p-4 shadow-sm">
            ...
            <p className="text-2xl font-bold text-foreground mt-1">{report.totalRespondents}</p>
          </div>
          <div className="bg-card rounded-xl border border-border p-4 shadow-sm">
            <p className="text-xs text-muted-foreground">Overall Satisfaction</p>
            <p className="text-2xl font-bold text-foreground mt-1">{report.overallPct.toFixed(1)}%</p>
          </div>
          <div className="bg-card rounded-xl border border-border p-4 shadow-sm">
              ...
              <div key={s.key} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-muted rounded-full h-5 relative">
                  <div
                    ...
                    style={{ width: `${s.pct}%` }}
                  >
                    <span className="text-[10px] font-bold text-primary-foreground">{s.pct.toFixed(1)}%</span>
                  </div>
                </div>
              </div>
            ))}
